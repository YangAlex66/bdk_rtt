...
dhcp_server_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *recv_addr, u16_t port)
{
    ...
    {
        DEBUG_PRINTF("dhcp_server_recv: recv_netif %c%c skip\n", recv_netif->name[0], recv_netif->name[1]);
        pbuf_free(p);
        return;
    }
    ...
    {
        DEBUG_PRINTF("DHCP request message or pbuf too short\n");
        pbuf_free(p);
        return;
    }
    ...
    if (q == NULL)
    {
        DEBUG_PRINTF("pbuf_alloc dhcp_msg failed!\n");
        pbuf_free(p);
        return;
    }
    if (q->tot_len < p->tot_len)
    {
        DEBUG_PRINTF("pbuf_alloc dhcp_msg too small %d:%d\n", q->tot_len, p->tot_len);
        pbuf_free(p);
        return;
    }
    ...
    {
        DEBUG_PRINTF("not a DHCP request message, but type %"U16_F"\n", (u16_t)msg->op);
        goto free_pbuf_and_return;
    }

    if (msg->cookie != PP_HTONL(DHCP_MAGIC_COOKIE))
    {
        DEBUG_PRINTF("bad DHCP_MAGIC_COOKIE!\n");
        goto free_pbuf_and_return;
    }

    if (msg->hlen > DHCP_MAX_HLEN)
    {
        DEBUG_PRINTF("bad DHCP_MAX_HLEN %d!\n", msg->hlen);
        goto free_pbuf_and_return;
    }
        ...
        if (msg_type == DHCP_DISCOVER)
        {
            ...
            *opt_buf++ = DHCP_OPTION_END;
            ...
            else
            {
                ...
                etharp_add_static_entry((ip4_addr_t *)&node->ipaddr, (struct eth_addr *)msg->chaddr);
                udp_sendto_if(pcb, q, &addr, port, dhcp_server->netif);
                etharp_remove_static_entry((ip4_addr_t *)&node->ipaddr);
            }
        }
        else
        {
...
dhcp_server_start(struct netif *netif, ip4_addr_t *start, ip4_addr_t *end)
{
    ...
    dhcp_server = NULL;
    DEBUG_PRINTF("dhcp_server_start(): starting new DHCP server\n");
    dhcp_server = (struct dhcp_server *)mem_malloc(sizeof(struct dhcp_server));
    if (dhcp_server == NULL)
    {
        DEBUG_PRINTF("dhcp_server_start(): could not allocate dhcp\n");
        return ERR_MEM;
    }
    ...
    {
        DEBUG_PRINTF("dhcp_server_start(): could not obtain pcb\n");
        return ERR_MEM;
    }
    ...
    DEBUG_PRINTF("dhcp_server_start(): starting DHCP server\n");
